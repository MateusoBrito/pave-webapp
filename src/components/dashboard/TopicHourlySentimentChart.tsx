...
/** Sentimento empilhado por hora — não por dia: o tópico já vive inteiro dentro de
 * um único dia calendário (modelagem diária), uma série diária colapsaria num ponto só. */
export function TopicHourlySentimentChart({ points, loading, error, refetch }: Props) {
    ...
    >
      <div className="flex items-center gap-[13px]">
        ...
        <div>
          ...
          <p className="text-[11px] text-[var(--text-muted)]">
            Distribuição por hora dos comentários sobre este tópico
          </p>
        </div>
      </div>
      ...
      {error ? (
        ...
        <StatusCard
          ...
          description="Ninguém falou sobre este tópico neste dia."
        />
      ) : (
      ...
      {!loading && !error && !isEmpty && (
        <div className="flex items-start gap-[10px] rounded-[11px] border border-[var(--tint-amber)] bg-[var(--tint-amber)] px-[14px] py-[11px]">
          <AlertTriangle size={15} className="mt-0.5 shrink-0 text-[var(--tint-text-amber)]" />
